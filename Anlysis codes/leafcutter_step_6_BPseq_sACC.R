...
load(here("leafcutter/data/covariates_results/step_5_BPseq_sACC.Rdata"))
...
walk( clusters$clusterID , ~make_cluster_plot(cluster_to_plot = .x ,exons_table = exons_table ,cluster_ids = cluster_ids , introns =  introns , meta = meta , counts = counts ,snp_pos <-  NA , file_path = here("graphs" , "BPseq_model" , "leafcutter" , region= "sACC" ,  paste0(.x , "_graph.pdf"))))
...
write.table(clusters, file = here("results" , "leafcutter" , paste0("BPseq_sACC" , "_sig_clusters.csv")) , sep = ",", 
...
write.table(intron_df, file = here("results" , "leafcutter" , paste0("BPseq_sACC" , "_sig_introns.csv")), sep = ",", 
            quote = FALSE, row.names = FALSE)

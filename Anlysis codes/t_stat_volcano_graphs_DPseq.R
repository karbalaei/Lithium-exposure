...
Lithium_gene <- read_csv(here("results" , "Lithium_DPseq_model_gene.csv")) %>% dplyr::rename(Id = ...1)
Lithium_exon <- read_csv(here("results" , "Lithium_DPseq_model_exon.csv")) %>% dplyr::rename(Id = ...1)
Lithium_Jxn <- read_csv(here("results" , "Lithium_DPseq_model_Jxn.csv")) %>% dplyr::rename(Id = ...1)
Lithium_Transcript <- read_csv(here("results" , "Lithium_DPseq_model_Transcript.csv")) %>% dplyr::rename(Id = ...1)
...
Volcano_t_graphs <-  function(df , type , cutoff , xlim1 , xlim2 , ylim1 ) {
 ...
 df_t%>%
  ...
  ggsave(filename = here("graphs",  "DPseq_model" , "t_stat" ,  paste0("Comparing t stat from Amygdala and sACC regions " , type ,  " .jpeg")) , width = 8 , height = 8 )
 ...
 ggsave( here("graphs",  "DPseq_model" , "volcano_plot" ,  paste0("Amygdala region " , type ,  " .jpeg")) , width = 8 , height = 8 )
 ...
 ggsave( here("graphs",   "DPseq_model" , "volcano_plot" ,  paste0("sACC region " , type ,  " ..jpeg")) , width = 8 , height = 8 )

...
Comparing BD patients with documented lithium exposure against non-exposed BD patients is clinically and scientifically crucial for several reasons:
...
The analytical pipeline comprises the following sequential components:

![Flowchart](assets/Flowchart.jpg) 

1. **Differential Expression Testing (Gene, Exon, Junction, and Transcript levels):** Executed utilizing the `limma::voom` and `lmFit` framework in R. Detailed scripts and parameters are available in the [`Differentially_Expressed_Features`](./Differentially_Expressed_Features/) directory.
2. **Alternative Splicing Event Detection:** Modeled annotation-free intron excision profiles using Leafcutter. Detailed workflows are documented in the [`Leafcutter`](./Leafcutter/) directory.
3. **Functional Annotation & Gene-Set Enrichment:** Identified overrepresented biological pathways utilizing the `enrichGO` function in R. Detailed parameters are available in the [`Gene_set_enrichment`](./Gene_set_enrichment/) directory.
4. **Weighted Gene Co-expression Network Analysis (WGCNA):** Constructed transcriptional networks and performed downstream module-trait association analyses. Detailed scripts are available in the [`WGCNA`](./WGCNA/) directory.
5. **Cross-Study Comparative Analysis:** Meta-analytic comparison evaluating the concordance of our differentially expressed gene lists against published historical benchmarks (*Still ongoing*).
6. **Data Visualization & Reporting:** Generation of publication-ready figures, volcano plots, and structured expression tables. Templates and code can be found in the [`Plots`](./Plots/) directory.

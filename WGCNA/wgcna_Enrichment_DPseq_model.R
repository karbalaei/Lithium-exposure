...
modJoint_lithium  = model.matrix(~lithium_group*BrainRegion + AgeDeath + Sex + snpPC1 + snpPC2 + snpPC3 + snpPC4 + snpPC5 +
                                 mitoRate + rRNA_rate + totalAssignedGene + RIN + abs(ERCCsumLogErr), 
                               data=colData(rse_gene))
...
modQsvaLithium = cbind(modJoint_lithium[,c(1:4,16,5:15)], qSV_mat_Lithium)
...
pdf(here::here("graphs" , "DPseq_model" ,  "wgcna" ,  "MEs_vs_Lithium.pdf"),w=8,h=6)
...
pdf(here::here("graphs" , "DPseq_model" , "wgcna" ,"clean_MEs_vs_Lithium.pdf"),w=5,h=6)
palette(brewer.pal(4,"Paired"))
par(mar=c(3,6,4,2), cex.axis=0.8,cex.lab=1,cex.main = 1.4)
...
load(here::here("results" ,  "Lithium results DPseqmodel.RDS"))

#rm(outExon_bothRegion, outJxn_bothRegion, outTx_bothRegion)

identical(rownames(Lithium_DPseqmodel$gene$Amygdala), fNames)

length(rownames(Lithium_DPseqmodel$gene$Amygdala))  #24784
length(fNames) #25136

sum(fNames %in% rownames(Lithium_DPseqmodel$gene$Amygdala)  ) #24784


sum(rownames(Lithium_DPseqmodel$gene$Amygdala) %in% fNames) #24784
...
Avi_id <-   which(net$fNames %in% rownames(Lithium_DPseqmodel[[i]][[j]]))

net_Lithium <-  list()

net_Lithium$colorsLab <-  net$colorsLab[Avi_id]

tt = table(net_Lithium$colorsLab, Lithium_DPseqmodel[[i]][[j]]$P.Value < 0.005)
...
deModEnrich = as.data.frame(t(sapply(colorDat$col, function(cc) {
  tab = table(net_Lithium$colorsLab == cc,Lithium_DPseqmodel[[i]][[j]]$P.Value < 0.005)
...
write.csv(prop.table_tt , here::here("results" , "wgcna_Enrichment" , "DPseq_model" , paste0("prop.table of  availability of DEf in WGCNA modules" , name , ".csv" ) ), row.names = T)

write.csv(deModEnrich , here::here("results" , "wgcna_Enrichment" , "DPseq_model" , paste0("Enrichment on WGCNA modules stat graphs " , name , ".csv" ) ), row.names = T)


jpeg(here::here("graphs" , "DPseq_model" , "wgcna" ,  paste0("Enrichment on WGCNA modules stat graphs " , name , ".jpg" ) ) , res = 300 , width = 5000 , height = 4000)
...
jpeg(here::here("graphs" , "DPseq_model" , "wgcna" ,  paste0("Enrichment DEfs on WGCNA modules stat graphs " , name , ".jpg" ) ) , res = 200 , width = 3000 , height = 4000)
...
jpeg(here::here("graphs" , "DPseq_model" , "wgcna" ,  paste0("prop.table of  availability of DEf in WGCNA modules " , name , ".jpg" ) ) , res = 200 , width = 3000 , height = 4000)
...
common_id <-   intersect(net$fNames , Lithium_DPseqmodel[[i]][[j]]$common_gene_id)

Avi_id <-   which(net$fNames %in%  common_id)

Lithium_DPseqmodel_common <-  Lithium_DPseqmodel[[i]][[j]][Lithium_DPseqmodel[[i]][[j]]$common_gene_id %in% common_id ,]

Lithium_DPseqmodel_DEf <-  Lithium_DPseqmodel_common[Lithium_DPseqmodel_common$P.Value < 0.005 , 'common_gene_id' ] %>% unique()

univers <-  Lithium_DPseqmodel_common$common_gene_id %>% unique() %>% as.data.frame() %>%
  setNames("Ids") %>% mutate("DEf" = if_else(Ids %in% Lithium_DPseqmodel_DEf , TRUE , FALSE))
...
Enrichment_fun_junction <- function( i , j , name) {
  ...
  common_id <-   intersect(net$fNames , unique(Lithium_DPseqmodel[[i]][[j]]$common_gene_id))
  ...
  jpeg(here::here("graphs" , "DPseq_model" , "wgcna" ,  paste0("Enrichment DEfs on WGCNA modules stat graphs " , name , ".jpg" ) ) , res = 400 , width = 3000 , height = 4000)
  ...
  jpeg(here::here("graphs" , "DPseq_model" , "wgcna" ,  paste0("prop.table of  availability of DEf in WGCNA modules " , name , ".jpg" ) ) , res = 400 , width = 3000 , height = 4000)

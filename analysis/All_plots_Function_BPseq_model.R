...
signif_genes <- map_depth(Lithium_BPseqmodel, 2, ~get_signif_genes(.x, colname = "common_gene_id", return_unique = TRUE))
map_depth(signif_genes, 2, length)


signif_features <- map_depth(Lithium_BPseqmodel, 2, ~get_signif_feature(.x,  return_unique = TRUE))
...
output_dir <- here::here("graphs", "BPseq_model", "Venn_diagram")
...
Other_RNA <-  c("snRNA","misc_RNA"  ,"sense_overlapping"  , "snoRNA" , "TEC" , "sense_intronic" , "processed_transcript" )

pseudogene_RNA <-  c("transcribed_unprocessed_pseudogene" , "transcribed_processed_pseudogene"  , "unprocessed_pseudogene"  ,          
                     "processed_pseudogene" , "polymorphic_pseudogene" )
